...
// SSC parsing code moved from solarsys.cpp 25.07.2019
        ...
        if (bodyType != UnknownBodyType)
        {
            ...
            if (parentSystem != nullptr)
            {
                ...
                if (body != nullptr && disposition == DataDisposition::Add)
                {
                    ...
                    {
                        body->addAlias(*iter, string(), parentSystem);
                        iter++;
                    }
                }
            }
...
bool SSCDataLoader::load(istream& in)
{
    return LoadSolarSystemObjects(in, *m_univ, resourcePath);
}
