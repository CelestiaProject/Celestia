                                      ...
                                      double scale) const
{
    ...
    double minDistance = (obsPosition - _cellCenterPos).norm() - scale * numbers::sqrt3;
    ...
    if (!_children || (minDistance > 0.0 && astro::absToAppMag(static_cast<double>(_exclusionFactor), minDistance) <= limitingFactor))
        return;
                                    ...
                                    double scale) const
{
    // Compute the distance to node; this is equal to the distance to
    // the cellCenterPos of the node minus the boundingRadius of the node, scale * SQRT3.
    double nodeDistance = (obsPosition - _cellCenterPos).norm() - scale * numbers::sqrt3;
