...
{
#ifdef USE_ICU
    for (const auto &[_, formatter] : localFormatters)
        udat_close(formatter);
    for (const auto &[_, formatter] : utcFormatters)
        udat_close(formatter);
#endif
}
...
UDateFormat *DateFormatter::getFormatter(bool local, astro::Date::Format format)
{
    ...
    auto iter = formatters.find(format);
    UDateFormat *formatter = nullptr;
    ...
    else
    {
        ...
        UErrorCode error = U_ZERO_ERROR;
        formatter = udat_open(timeStyle, dateStyle, locale, local ? nullptr : u"UTC", -1, pattern, -1, &error);
        if (U_FAILURE(error))
            return nullptr;
        formatters.insert(std::pair<astro::Date::Format, UDateFormat*>(format, formatter));
    }
    return formatter;
}
#endif
