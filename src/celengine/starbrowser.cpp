...
public:
    ...
    StarBrowserRecord createRecord(const Star*) const;
    void finalizeRecord(StarBrowserRecord&) const { /* no-op */ }
...
StarFilter::operator()(const Star* star) const
{
    ...
    {
        const auto end = m_solarSystems->end();
        if (visibleOnly)
        {
            // When searching for visible stars only, also take planets orbiting the
            // parent barycenters into account
            const Star* hostStar = star;
            for (;;)
            {
                if (m_solarSystems->find(hostStar->getIndex()) != end)
                    break;

                hostStar = hostStar->getOrbitBarycenter();
                if (hostStar == nullptr || hostStar->getVisibility())
                    return false;
            }
        }
        else if (m_solarSystems->find(star->getIndex()) == end)
        {
            return false;
        }
    }
...
void
StarBrowser::populate(std::vector<StarBrowserRecord>& records)
{
