...
{
    StarBrowserRecord(const Star* _star) : star(_star) {}
    const Star* star;
...
public:
    ...
    void setSpectralTypeFilter(std::function<bool(const char*)> filter) { m_spectralTypeFilter = filter; }
    ...
    void populate(std::vector<StarBrowserRecord>&);
