...
constexpr std::uint16_t StarDBVersion = UINT16_C(0x0100);
constexpr std::uint16_t CrossIndexVersion = UINT16_C(0x0100);
...
parseCustomStarDetails(const AssociativeArray* starData,
                       const fs::path& path)
{
                                ...
                                bool isBarycenter)
{
    ...
    else
    {
        ...
        if (modifyPosition)
        {
            ...
            auto raf = static_cast<float>(ra);
            auto decf = static_cast<float>(dec);
            auto distancef = static_cast<float>(distance);
