...
static int CALLBACK StarBrowserCompareName(LPARAM lParam0, LPARAM lParam1, LPARAM lParamInfo)
{
    ...
    auto stardb = reinterpret_cast<const StarDatabase*>(lParamInfo);

    starNameString = util::UTF8ToCurrentCP(stardb->getStarName(*record0->star, true));
    starNameString2 = util::UTF8ToCurrentCP(stardb->getStarName(*record1->star, true));
    return CompareString(LOCALE_USER_DEFAULT, NORM_LINGUISTIC_CASING | SORT_DIGITSASNUMBERS,
                         starNameString.data(), static_cast<int>(starNameString.size()),
                         starNameString2.data(), static_cast<int>(starNameString2.size()));
}

static int CALLBACK StarBrowserCompareDistance(LPARAM lParam0, LPARAM lParam1, LPARAM)
{
    auto record0 = reinterpret_cast<const engine::StarBrowserRecord*>(lParam0);
    auto record1 = reinterpret_cast<const engine::StarBrowserRecord*>(lParam1);

    return static_cast<int>(math::sign(record0->distance - record1->distance));
}

static int CALLBACK StarBrowserCompareAppMag(LPARAM lParam0, LPARAM lParam1, LPARAM)
{
    auto record0 = reinterpret_cast<const engine::StarBrowserRecord*>(lParam0);
    auto record1 = reinterpret_cast<const engine::StarBrowserRecord*>(lParam1);
    return static_cast<int>(math::sign(record0->appMag - record1->appMag));
}

static int CALLBACK StarBrowserCompareAbsMag(LPARAM lParam0, LPARAM lParam1, LPARAM)
{
    auto record0 = reinterpret_cast<const engine::StarBrowserRecord*>(lParam0);
    auto record1 = reinterpret_cast<const engine::StarBrowserRecord*>(lParam1);
    return static_cast<int>(math::sign(record0->star->getAbsoluteMagnitude() - record1->star->getAbsoluteMagnitude()));
}

static int CALLBACK StarBrowserCompareSpectralType(LPARAM lParam0, LPARAM lParam1, LPARAM)
{
    auto record0 = reinterpret_cast<const engine::StarBrowserRecord*>(lParam0);
    auto record1 = reinterpret_cast<const engine::StarBrowserRecord*>(lParam1);
    return std::strcmp(record0->star->getSpectralType(), record1->star->getSpectralType());
}
...
void StarBrowserDisplayItem(LPNMLVDISPINFOA nm, StarBrowser* browser)
{
    ...
    case 1:
        {
            Vector3d r = record->star->getPosition(browser->starBrowser.time()).offsetFromKm(browser->starBrowser.position());
    ...
    case 2:
        {
            Vector3d r = record->star->getPosition(browser->starBrowser.time()).offsetFromKm(browser->starBrowser.position());
            float appMag = record->star->getApparentMagnitude(astro::kilometersToLightYears(r.norm()));
    ...
    case 3:
        sprintf(callbackScratch, "%.2f", record->star->getAbsoluteMagnitude());
        nm->item.pszText = callbackScratch;
        break;

    case 4:
        strncpy(callbackScratch, record->star->getSpectralType(), sizeof(callbackScratch)); /* Flawfinder: ignore */
                ...
                case LVN_COLUMNCLICK:
                    {
                        ...
                        if (hwnd != 0)
                        {
                            ...
                            {
                                const StarDatabase* stardb = browser->starBrowser.universe()->getStarCatalog();
                                ListView_SortItems(hwnd, compareFuncs[subItem], reinterpret_cast<LPARAM>(stardb));
                            }
                        }
                    }
